...
fg_api.prototype.adsModule = function() {
	...
	module.prototype.showIMA = function(callback, force) {
		...
		if (detection.is.pc) {
		    M.adWidth = Math.min(800, window.innerWidth) + "px";
		}
		...
		try {
			M.showAdModal({
				"overlayStyle": {
				    'background': '#000 url("' + self.config.blurred_thumb + '") no-repeat center center', 
			...
			switch (adEvent.type) {
				case google.ima.AdEvent.Type.LOADED:
					...
					if (ad.isLinear() && M.adsManager.getRemainingTime() > 0) {
						self.modal.setCloseBtnTimer(30);
					}
	...
	module.prototype.getAdTagUrl = function(variant) {
		...
		M.dfp_custom_params = {
			...
			"video": (M.show_video) ? "1" : "0", // allow video for desktop always; on mobile only for request A) because of the neccessary user interaction
...
fg_api.prototype.navigationModule = function() {
	...
	navigationPrototype.getMoreGamesLink = function() {
		return "javascript:void(0);"; //self.__("more_games_url");
	};
...
fg_api.prototype.trackingModule = function() {
	...
	trackingPrototype.init = function() {
		...
		if (!self.hasFeature("standalone")) {
			faZepto.getScript("https://staging.api.html5games.com/js/html5games_api.js", function() {
				if (typeof html5games_api !== "undefined") {
					html5games_api.init("famobi/" + self.config.package_id);
				}
			});

			// https://codepen.io/anon/pen/mpgOXa?limit=all&page=2&q=counter
		}
	};
...
fg_api.prototype.firebaseModule = function() {
	...
	module.prototype.initializeFirebase = function() {
		...
		M.promise = new Promise(function(resolve, reject) {
			...
			faZepto.getScript('https://www.gstatic.com/firebasejs/4.8.2/firebase.js', function() {
				// Initialize Firebase
				var config = {
